...
namespace SerialPortUtility
{
	...
	public class SerialPortUtilityPro : MonoBehaviour
	{
		...
		void Start()
		{
			...
			Debug.Log("android plugin: "+_androidPlugin);
#else
            //Unlock
            if (!licensecheck) {
		...
		private System.Collections.IEnumerator OpenProcessing()
		{
			...
			if(_androidPlugin != null)
			{
				...
				//		break;
				//}
				Debug.Log(BaudRate+" "+(int)Parity+" "+(int)DataBit+" "+(int)StopBit+" "+(RecvDiscardNull == true)+" "+(RecvDiscardNull == true)+" "+(RecvIgnoreBreakSignal == true)+" "+Skip);
				SerialPortHandleSet = _androidPlugin.Call<int>("OpenUSB", VendorID, ProductID, SerialNumber_search_string, config);
				Debug.Log("called hardcoded: "+ SerialPortHandleSet);
			}
#else
            //Start
            SpapConfig config;
...
#if UNITY_ANDROID && !UNITY_EDITOR
            sbyte[] sWriteData = System.Array.ConvertAll(writeData, b => unchecked((sbyte)b)); ;
			if(_androidPlugin != null)
				_androidPlugin.Call("WriteBuffer", SerialPortHandle, sWriteData, writeData.Length);
#else
        ...
        public static DeviceInfo[] GetConnectedDeviceList(OpenSystem device_format)
        {
            ...
            else
            {
				...
				throw new System.Exception("GetConnectedDeviceList is NOT SUPPORTED!");
            }
#else
